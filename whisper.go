...
/*
#cgo LDFLAGS: -lwhisper -lm -lstdc++ -lggml -fopenmp
...
type (
	Context_C          C.struct_whisper_context
	Token_C            C.whisper_token
	TokenData_C        C.struct_whisper_token_data
	SamplingStrategy_C C.enum_whisper_sampling_strategy
	Params_C           C.struct_whisper_full_params
)
...
const (
	SAMPLING_GREEDY      SamplingStrategy_C = C.WHISPER_SAMPLING_GREEDY
	SAMPLING_BEAM_SEARCH SamplingStrategy_C = C.WHISPER_SAMPLING_BEAM_SEARCH
)
...
func (ctx *Context_C) Whisper_free() {
	C.whisper_free((*C.struct_whisper_context)(ctx))
}

// Convert RAW PCM audio to log mel spectrogram.
// The resulting spectrogram is stored inside the provided whisper context.
func (ctx *Context_C) Whisper_pcm_to_mel(data []float32, threads int) error {
...
// n_mel must be 80
func (ctx *Context_C) Whisper_set_mel(data []float32, n_mel int) error {
...
func (ctx *Context_C) Whisper_encode(offset, threads int) error {
...
func (ctx *Context_C) Whisper_decode(tokens []Token_C, past, threads int) error {
...
func (ctx *Context_C) Whisper_tokenize(text string, tokens []Token_C) (int, error) {
...
//	"de" -> 2
//	"german" -> 2
func (ctx *Context_C) Whisper_lang_id(lang string) int {
...
func (ctx *Context_C) Whisper_lang_auto_detect(offset_ms, n_threads int) ([]float32, error) {
...
// Token_C Id -> String. Uses the vocabulary in the provided context
func (ctx *Context_C) Whisper_token_to_str(token Token_C) string {
	return C.GoString(C.whisper_token_to_str((*C.struct_whisper_context)(ctx), C.whisper_token(token)))
}

// Special tokens
func (ctx *Context_C) Whisper_token_eot() Token_C {
	return Token_C(C.whisper_token_eot((*C.struct_whisper_context)(ctx)))
}

// Special tokens
func (ctx *Context_C) Whisper_token_sot() Token_C {
	return Token_C(C.whisper_token_sot((*C.struct_whisper_context)(ctx)))
}

// Special tokens
func (ctx *Context_C) Whisper_token_prev() Token_C {
	return Token_C(C.whisper_token_prev((*C.struct_whisper_context)(ctx)))
}

// Special tokens
func (ctx *Context_C) Whisper_token_solm() Token_C {
	return Token_C(C.whisper_token_solm((*C.struct_whisper_context)(ctx)))
}

// Special tokens
func (ctx *Context_C) Whisper_token_not() Token_C {
	return Token_C(C.whisper_token_not((*C.struct_whisper_context)(ctx)))
}

// Special tokens
func (ctx *Context_C) Whisper_token_beg() Token_C {
	return Token_C(C.whisper_token_beg((*C.struct_whisper_context)(ctx)))
}

// Special tokens
func (ctx *Context_C) Whisper_token_lang(lang_id int) Token_C {
	return Token_C(C.whisper_token_lang((*C.struct_whisper_context)(ctx), C.int(lang_id)))
}

// Task tokens
func (ctx *Context_C) Whisper_token_translate() Token_C {
	return Token_C(C.whisper_token_translate((*C.struct_whisper_context)(ctx)))
}

// Task tokens
func (ctx *Context_C) Whisper_token_transcribe() Token_C {
	return Token_C(C.whisper_token_transcribe((*C.struct_whisper_context)(ctx)))
}

// Performance information
func (ctx *Context_C) Whisper_print_timings() {
	C.whisper_print_timings((*C.struct_whisper_context)(ctx))
}

// Performance information
func (ctx *Context_C) Whisper_reset_timings() {
...
func (ctx *Context_C) Whisper_full_default_params(strategy SamplingStrategy_C) Params_C {
	// Get default parameters
	return Params_C(C.whisper_full_default_params_cb((*C.struct_whisper_context)(ctx), C.enum_whisper_sampling_strategy(strategy)))
}

// Run the entire model: PCM -> log mel spectrogram -> encoder -> decoder -> text
// Uses the specified decoding strategy to obtain the text.
func (ctx *Context_C) Whisper_full(
	params Params_C,
	samples []float32,
...
func (ctx *Context_C) Whisper_full_parallel(params Params_C, samples []float32, processors int, encoderBeginCallback func() bool, newSegmentCallback func(int)) error {
	registerEncoderBeginCallback(ctx, encoderBeginCallback)
	registerNewSegmentCallback(ctx, newSegmentCallback)
	defer registerEncoderBeginCallback(ctx, nil)
	defer registerNewSegmentCallback(ctx, nil)
...
func (ctx *Context_C) Whisper_full_get_token_text(segment int, token int) string {
...
func (ctx *Context_C) Whisper_full_get_token_id(segment int, token int) Token_C {
	return Token_C(C.whisper_full_get_token_id((*C.struct_whisper_context)(ctx), C.int(segment), C.int(token)))
}

// Get token data for the specified token in the specified segment.
// This contains probabilities, timestamps, etc.
func (ctx *Context_C) Whisper_full_get_token_data(segment int, token int) TokenData_C {
	return TokenData_C(C.whisper_full_get_token_data((*C.struct_whisper_context)(ctx), C.int(segment), C.int(token)))
}

// Get the probability of the specified token in the specified segment.
func (ctx *Context_C) Whisper_full_get_token_p(segment int, token int) float32 {
...
func registerNewSegmentCallback(ctx *Context_C, fn func(int)) {
	if fn == nil {
...
func registerProgressCallback(ctx *Context_C, fn func(int)) {
	if fn == nil {
...
func registerEncoderBeginCallback(ctx *Context_C, fn func() bool) {
	if fn == nil {
...
func (t TokenData_C) T0() int64 {
	return int64(t.t0)
}

func (t TokenData_C) T1() int64 {
	return int64(t.t1)
}

func (t TokenData_C) Id() Token_C {
	return Token_C(t.id)
}
