...
func newContext(model *model, params Params_C) (Context, error) {
	context := new(context)
	context.model = model
...
func (context *context) IsText(t Token) bool {
	...
	case context.IsSOT(t):
		return false
	case Token_C(t.Id) >= context.model.ctx.Whisper_token_eot():
		return false
